...
const db = SQLite.openDatabase("tasks.db", VERSION);
...
export const initDatabase = callback => {
  db.transaction(
    tx => {
      ...
      tx.executeSql(
        "INSERT OR IGNORE INTO settings (id, sorting, sortingType, timeFormat, firstDayOfWeek, confirmFinishingTask, confirmRepeatingTask, confirmDeletingTask, version, theme, lang) values (0, 'byAZ', 'ASC', 1, 'Sunday', 1, 1, 1, ?, 0, 'en');",
        [VERSION + "_INIT"],
        () => {
          initApp(callback);
        }
